...
public class PopularRooms extends IMessage
{
	@Override
	public void handle()
	{
		session.sendResponse(new RoomDataComposer(CategoryEngine.getRoomCategories().get(0).RoomInfo));
		
		ConcurrentLinkedQueue<Room> activeRooms = new ConcurrentLinkedQueue<Room>();

		for (Room Room : RoomEngine.roomMap().values())
			if (Room.getUsers().size() != 0)
				activeRooms.add(Room);

		session.sendResponse(new NavigatorListComposer(activeRooms));
	}
}
