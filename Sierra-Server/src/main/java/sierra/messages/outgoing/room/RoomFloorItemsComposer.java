...
public class RoomFloorItemsComposer extends ICompose {
	...
	public Response compose() {
		...
		response.appendInt32(this.Room.getFloorItems().size());
		
		for (FloorItem Item : this.Room.getFloorItems()) {
			response.appendBody(Item);
			response.appendInt32(this.Room.getOwnerId());
		}
